package com.alidev.android.portfolio.opengl;
...
class EglHelper {
   ...
   SurfaceTexture createSurface(SurfaceTexture surfaceTexture, boolean isVideo) {
      ...
      if (eglConfig != null) {
         ...
         if (EGL14.eglGetError() != EGL14.EGL_SUCCESS) {
            Logger.e("Failed to create EGL3 context");
      ...
      Logger.d("EGLContext created, client version %d", values[0]);
   ...
   private EGLConfig createEGLConfig(int version, boolean isVideo) {
      ...
      if (!EGL14.eglChooseConfig(eglDisplay, attributeList, 0, configs, 0, configs.length, numConfigs, 0)) {
         Logger.e("unable to find RGB8888 %d EGLConfig", version);
         return null;
      }
      return configs[0];
   }
   ...
   boolean swapBuffers() {
      ...
      if (!success) {
         Logger.e("eglSwapBuffers failed");
      }
      return success;
   }
